...
const props = defineProps(["eleicao"]);
const form = useForm({
    Ano: props.eleicao.Ano,
    Descricao: props.eleicao.Descricao
});
...
<template>
    ...
    <AuthBase title="Edite a eleição" description="Preencha as informações necessárias">
        ...
        <form @submit.prevent="form.put(route('update-eleicao', {id: eleicao.id}))" class="flex flex-col gap-6">
            <div class="grid gap-6">
                <div class="grid gap-2">
                    <Label for="Ano">Ano da eleição</Label>
                    <Input id="Ano" type="text" required autofocus :tabindex="1" v-model="form.Ano" placeholder="Ano da eleição" />
